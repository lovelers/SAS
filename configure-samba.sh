#!/bin/bash
# Samba Configurer for Linux
# Tested in Arch Linux and Ubuntu Server.
...
echo "Samba Configuration Script by Luke Channings"
echo "If you want to add samba users make sure to run the script as root."

## Ask where to save the script.

read -p "Where would you like the $file file to be saved? (Default $defaultplace) " dir

if [ -z "$dir" ]; then
  saveplace=$defaultplace ## Default to /etc/samba
fi

if [ "$dir" = "~" ]; then dir=$HOME; fi ## If a tilde is passed replace it with $HOME

## BEGIN QUESTIONS FOR GENERAL SAMBA CONFIGURATION.

# NetBios
read -p "Please enter your NetBios Name (Default is $host) " netbios
if [ -z "$netbios" ]; then netbios=$host; fi ## If no netbios name is passed default to the hostname.

# Server String
read -p "Please enter server description (Default is the Samba version) " serverString
if [ -z "$serverString" ]; then serverString="Samba version %v"; fi ## Default to Samba Version.

# Workgroup
read -p "Please enter a workgroup name (Default is WORKGROUP) " workgroup
if [ -z "$workgroup" ]; then workgroup="WORKGROUP"; fi ## Default to WORKGROUP

# Security Mode
read -p "Please set a security mode. Options are \"user\", \"share\" and \"domain\" (Default is user)" security
